...
class SheetsDeleteRowsColsModule @Inject constructor(
    ...
    override suspend fun execute(context: ExecutionContext): ExecutionResult {
        return try {
            ...
            if (indicesToDelete.isNotEmpty()) {
                // Group indices and sort descending to avoid index shifting issues
                val ranges = groupIndices(indicesToDelete).sortedByDescending { it.first }
            ...
            ExecutionResult.Success("Deleted ${indicesToDelete.size} $dimension(s) in sheet: $sheetName")
        } catch (e: Exception) {
            Timber.e(e, "Failed to delete rows/cols")
            ExecutionResult.Error("Failed to delete rows/cols: ${e.message}")
        }
    }
